import { konten, systemSchemas } from "./dbtools";
...
import {WebsiteAnfrage, Aktion, ServerStatus} from './server'
...
export async function decideOnAktion(anfrage: WebsiteAnfrage, aktionen: Aktion[]): Promise<Aktion | null>{    
    try {
...
export async function getInfoAnswer(serverStatus: any): Promise<string>{
    try {
...
export async function decideOnGoogleSearch(serverStatus: ServerStatus): Promise<{text: string, beschreibung: string}>{
...
export async function decideOnQuery(serverStatus: any): Promise<{query: string, beschreibung: string}>{
    ...
    let prompt = `Du bist eine clevere AI, die Teil eines ausgeklügelten Systems ist, um sämtliche Anfragen eines Nutzers einer Website
        ...
        if(serverStatus.aktion.name === "Finanzen"){
            ...
            Die MongoDB umfasst folgende Collections mit folgenden AJV-Formata: ${JSON.stringify(systemSchemas)}

            Das hier sind alle vorhandenen Konten des Nutzers: ${JSON.stringify(konten)}
            ...
            **Beispiel 3**
            ...
            prompt += `Die MongoDB umfasst folgende Collections mit folgenden AJV-Formata: ${JSON.stringify(systemSchemas)}
...
export async function decideOnNextStep(serverStatus: ServerStatus): Promise<{aktion: "UserInput" | "Datenbank"| "Google", aktion_data: any}>{
    try {
...
export async function decideOnWhatToAskUser(serverStatus: any): Promise<string>{
...
export async function decideOnSchema(anfrage: string): Promise<string>{
    try {
...
export async function decideOnEnoughInfoToContinue(serverStatus: ServerStatus): Promise<boolean>{
...
export async function decideOnEnoughInfoToCreate(command: string, object: any, schema: string): Promise<{enough: boolean, missingFields: any}>{
    try {
...
export async function decideOnUpdatedObject(anfrage: string, currentDocument: {id: number, collection: string, data: any}, verlauf: any[]): Promise<any>{
    try {
...
export async function decideOnObjectToCreate(command: string, verlauf: any[], schema: string): Promise<any>{
    try {
        let prompt
        if(schema !== "object"){
...
export async function tryToCreateSchemaObject(command: string, verlauf: any[], schema: string): Promise<{valid: boolean, object: any}>{
...
export async function tryToCreateBuchung(serverStatus: ServerStatus): Promise<{valid: boolean, object: any}>{
...
export async function prompt(promptText: string) {
    try {
        let responseObject = await openai.chat.completions.create({
            ...
            max_tokens: 150,
            top_p: 0.85,
            frequency_penalty: 0.5, // Helps in reducing repetition
...
export async function getTwitterCredentialsFromRequest(serverStatus: ServerStatus): Promise<{valid: boolean, username: string, password: string, userhandle: string}>{
    try {
        let promptText = `
...
export async function decideIfCrypto(tweetText: string) {
    try {
        let promptText = `
