...
export let dbClient;
export let systemConstants;
export let systemSchemas;
export let tabellen;
export let konten;
...
export async function getNextId() {
    try {
        const collection = dbClient.collection('sys_var');
        const result = await collection.findOneAndUpdate({ _id: new ObjectId(systemConstants._id) }, { $inc: { id_count: 1 } }, { returnDocument: 'after' });
        if (!result.value) {
...
export async function createRecord(record, table) {
    try {
        ...
        await dbClient.collection(table).insertOne(Object.assign(Object.assign({}, record), { id: recordId }));
        return recordId;
    }
    catch (error) {
        return -1;
